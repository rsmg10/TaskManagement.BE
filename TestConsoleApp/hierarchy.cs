...
public static class HierarchyIdSample
{
    public static async Task SQL_Server_HierarchyId()
    {
        ...
        if (!(await context.Halflings.AnyAsync()))
            await context.Seed();
        ...
        while (true)
        {
            ...
            var s = await context.Halflings.ToListAsync();
            var first = s.FirstOrDefault();
            var last = s.LastOrDefault();
            var hid = last.PathFromPatriarch;
            var lvl = hid.GetLevel();
            var anotherOne = hid.GetHashCode();
            // var anscestor = hid.GetAncestor();

            var queryString =  context.Halflings.Where(halfling => halfling.PathFromPatriarch.GetLevel() == level)
                .ToQueryString();
            var query = context.Halflings.AsQueryable();
            query = query.Where(halfling => halfling.PathFromPatriarch.GetLevel() == level);
            var list = await query.ToListAsync();
            
            var generation = await context.Halflings.Where(halfling => halfling.PathFromPatriarch.GetLevel() == level)
                .ToListAsync();
        ...
        var ponto = await context.Halflings.SingleAsync(halfling =>
            halfling.Name == "Ponto" && halfling.YearOfBirth == 1216);
    ...
    public class FamilyTreeContext : DbContext
    {
        ...
        {
            // modelBuilder.Entity<Halfling>(h =>
            //     h.Property(p => p.PathFromPatriarch).HasConversion(o => o.ToString(), s => HierarchyId.Parse(s)));
        }
